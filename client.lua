...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if IsNearMechanic() then
			...
			QBCore.Functions.Progressbar("Repairing Car", "Repairing Car", 10000, false, true, {
			...
			TSE('cash:repair')
			--exports['mythic_notify']:SendAlert('type', 'Your Vehicle Has Been Repaired ₹800')
			--notification("~g~The mechanic has repaired your car!")
			--local xPlayer = ESX.GetPlayerFromId(src)
            --xPlayer.removeBank(800)
			return
		end
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				FreezeEntityPosition(vehicle, true)
				QBCore.Functions.Progressbar("Repairing Car", "Repairing Car", 3000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
